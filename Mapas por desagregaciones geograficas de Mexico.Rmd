...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, cache = TRUE, cache.lazy = FALSE, collapse = TRUE, class.source = "fold-hide")
```
...
# Primero se deben instalar las paqueterías, por ello se instala el paquete pacman para descargar los demás paquetes con la función pacman::p_load().
# install.packages("pacman")
require(pacman)
pacman::p_load(dplyr, forcats, ggplot2, ggthemes, ggmap, ggpubr, knitr, kableExtra, lubridate, openxlsx, readxl, RColorBrewer, rgdal, sp, spdplyr, tidyverse, tmap, tmaptools, tibble, viridisLite, ggimage, grDevices)
...
##### $$SpatialPolygons \Rightarrow tbl\_df/ tbl / data.frame$$    
...
Se define como **localidad**: Todo lugar ocupado con una o más viviendas, las cuales pueden estar habitadas o no; este lugar es reconocido por un nombre dado por la ley o la costumbre.  
De acuerdo con sus características y con fines estadísticos, las localidades se clasifican en `urbanas` y `rurales`. Dado por el número de habitantes de una localidad determinara si es rural o urbana. De acuerdo con el INEGI, una población se considera rural cuando tiene menos de 2,500 habitantes, mientras que la urbana es aquella donde viven más de 2,500 personas.     

A continuación, se lee el archivo .shp que contiene de manera integrada la división del área geoestadística por $localidad urbana$ consolidada por polígonos.     
...
La función `rename()` del paquete `dplyr` permite cambiar el nombre de la columna de la clave geoestadística a nivel localidad urbana dentro de la base de datos del shape.        
...
A continuación se lee el archivo .shp que contiene de manera integrada la división de el área geoestadística por  $localidad rural$ consolidada por puntos.  

```{r,results=FALSE,class.source = "fold-show"}
shape_localidad_rur <- readOGR(dsn ="MGN 2020/conjunto_de_datos", 
                                layer = "00lpr",
                                 encoding = "UTF-8",
                                  use_iconv = TRUE)
```
...
require(ggmap)
register_google(key = "AIzaSyDtqSmCTS0DVBWGnmiAhI_cGi2BTgw4zUA")
```
...
#path = "Images/IME_2020_ggmap_API.pdf"
#ggexport (p, width = 8, height = 5, res = 400, filename = path)
print(p)
```
